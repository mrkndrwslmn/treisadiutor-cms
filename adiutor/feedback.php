...
$query = "SELECT f.*, s.fullName as sender_name, t.title as task_title 
          ...
          WHERE f.receiver_id = $adiutorId";

// Add filtering conditions
if ($filterRating > 0) {
    $query .= " AND f.rating = $filterRating";
}

if ($filterDate) {
    $query .= " AND DATE(f.created_at) = '$filterDate'";
}
...
$countQuery = str_replace("SELECT f.*, s.fullName as sender_name, t.title as task_title", "SELECT COUNT(*)", $query);
...
$avgRatingQuery = "SELECT AVG(rating) as avg_rating FROM feedbacks WHERE receiver_id = $adiutorId";
...
$ratingDistribution = [];
$ratingQuery = "SELECT rating, COUNT(*) as count FROM feedbacks WHERE receiver_id = $adiutorId GROUP BY rating ORDER BY rating DESC";
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <form action="" method="get" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-3 flex justify-end">
                ...
                <a href="feedback.php" class="ml-2 inline-flex items-center px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
