...
function formatPhone($phone) {
    if (!empty($phone) && strlen($phone) === 13 && substr($phone, 0, 3) === '+63') {
        return '+63 ' . substr($phone, 3, 3) . ' ' . substr($phone, 6, 3) . ' ' . substr($phone, 9);
    }
    return $phone;
}

// Get all users with role 'client' (case-insensitive, allow 'client' or 'clients')
$clients_query = "SELECT * FROM users WHERE role IN ('client')";
...
if ($clients_result && $clients_result->num_rows > 0) {
    while ($client = $clients_result->fetch_assoc()) {
        ...
        // Get average rating for this client (from this adiutor)
        $rating_query = "SELECT AVG(rating) as avg_rating FROM feedbacks WHERE giver_id = ?";
        $rating_stmt = $conn->prepare($rating_query);
        $rating_stmt->bind_param("i", $client['userID']);
        $rating_stmt->execute();
        $rating_result = $rating_stmt->get_result();
        $rating_data = $rating_result->fetch_assoc();

        // Get count of tasks for this client (assigned to this adiutor)
        $task_query = "SELECT COUNT(*) as task_count FROM tasks t 
                      JOIN forms f ON t.formID = f.formID 
                      WHERE f.userID = ?";
        $task_stmt = $conn->prepare($task_query);
        $task_stmt->bind_param("i", $client['userID']);
        $task_stmt->execute();
    ...
    // echo "<!-- No clients found in users table with role 'client' or 'clients' -->";
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="client-card bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
                ...
                <div class="p-4">
                    <div class="flex items-center mb-2">
                        ...
                        <span class="ml-2"><?php echo !empty($client['phoneNumber']) ? formatPhone($client['phoneNumber']) : 'N/A'; ?></span>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.view-client').forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch('ajax/get_client_details.php?client_id=' + clientId)
